...
const stripHtml = (html: string | null | undefined, maxLength?: number): string => {
  ...
  const filteredRequests = requests.filter(r => {
    ...
    if (activeTab === 'open') {
      ...
      tabMatch = r.sorumluSicil === user?.sicilNo && durumText !== 'Kapalı' && durumText !== 'KAPATILDI';
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f8fafc' }}>
      <ListHeader
        ...
        rightAction={{ icon: 'stats-chart-outline', onPress: () => navigation.navigate('HelpDeskDashboard', { tur: type, title: 'IT HelpDesk' }) }}
      >
