...
const stripHtml = (html: string | null | undefined, maxLength?: number): string => {
  ...
  export const BakimHelpDeskScreen = () => {
  ...
  const [isWorkOrderCreateModalOpen, setIsWorkOrderCreateModalOpen] = useState(false);
  const [isWorkOrderCloseModalOpen, setIsWorkOrderCloseModalOpen] = useState(false);
  const [isWorkOrderAssignModalOpen, setIsWorkOrderAssignModalOpen] = useState(false);
  const [selectedWorkOrder, setSelectedWorkOrder] = useState<any>(null);
  
  const [woFormTur, setWoFormTur] = useState<number | null>(null);
  const [woFormTermin, setWoFormTermin] = useState<Date | null>(null);
  ...
  useEffect(() => {
    if (isFocused) {
      api.getIsEmriTurleri().then(res => {
        setIsEmriTurleri(res);
      }).catch(err => console.error('IsEmriTurleri error:', err));
    }
  }, [isFocused]);
  ...
  // Bildirim dokunuşundan detay aç — talepKodu (code) öncelikli, id fallback.
  useEffect(() => {
  ...
  const handleCreateWorkOrder = async () => {
    ...
    try {
      ...
      Alert.alert('Hata', e?.response?.data?.message || 'İş emri oluşturulamadı.');
    }
  };
  ...
  const handleCloseWorkOrder = async () => {
    ...
    try {
      const res = await api.closeIsEmri(selectedWorkOrder?.talepIsEmriID as number, {
        aciklama: woFormAciklama
      });
      if (res.success) {
        Alert.alert('Başarılı', 'İş emri kapatıldı.');
        setIsWorkOrderCloseModalOpen(false);
        setWoFormAciklama('');
        if (selectedRequest?.talepID) loadRequestDetails(selectedRequest.talepID);
      }
    } catch (e: any) {
      Alert.alert('Hata', e?.response?.data?.message || 'İş emri kapatılamadı.');
    }
  };

  const handleAssignWorkOrder = async (sicil: string) => {
    try {
      const res = await api.assignIsEmri(selectedWorkOrder?.talepIsEmriID as number, {
        sicil: sicil
      });
      if (res.success) {
        Alert.alert('Başarılı', 'İş emri atandı.');
        setIsWorkOrderAssignModalOpen(false);
        if (selectedRequest?.talepID) loadRequestDetails(selectedRequest.talepID);
      }
    } catch (e: any) {
      Alert.alert('Hata', e?.response?.data?.message || 'İş emri atanamadı.');
    }
  };
  ...
  const handleKontrolKaydet = async () => {
    ...
    try {
      const res = await api.saveTalepKontrol(selectedRequest?.talepKodu as string, {
        ...
        yag: chkYag,
        miknatis: chkMiknatis,
        fazlaParca: chkFazlaParca,
        guvenlik: chkGuvenlik,
        makine: chkMakine,
        temizlik: chkTemizlik,
        gida: chkGida
      });
      if (res.success) {
  ...
  const formatStatusText = (text: string | null | undefined): string => {
    if (!text) return 'Açık';
    return text.replace(/A\?ik/g, 'Açık').replace(/Kapal\?/g, 'Kapalı').replace(/A\?ık/g, 'Açık');
  };
    ...
    <View style={{ flex: 1, backgroundColor: '#f8fafc' }}>
              ...
              {type === 'BAKIM' && (
                <View style={styles.bakimFieldsCard}>
                  ...
                  <View style={styles.formGroup}>
                    ...
                    >
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <Ionicons name="briefcase-outline" size={18} color={slateTokens.textMuted} />
                        <Text style={styles.selectBoxText}>
                  ...
                  <View style={styles.formGroup}>
                    ...
                    >
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <Ionicons name="cog-outline" size={18} color={slateTokens.textMuted} />
                        <Text style={styles.selectBoxText}>
                          {(bakimDropdowns?.makines || []).find((m: any) => m.makineKodu === formMakine)?.makineAdi || 'Makine Seçiniz'}
                        </Text>
                      </View>
                      <Ionicons name="chevron-down" size={18} color={slateTokens.textMuted} />
                    </TouchableOpacity>
                  ...
                  <View style={styles.formGroup}>
                    ...
                    <TouchableOpacity style={styles.selectBox} onPress={() => setIsDurusSelectOpen(true)}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <Ionicons name="stopwatch-outline" size={18} color={slateTokens.textMuted} />
                        <Text style={styles.selectBoxText}>
                          {formDurus === 'H' ? 'DURUŞ YOK' : (formDurus === 'EMAK' ? 'MAKİNE DURDU' : 'HAT DURDU')}
                        </Text>
                      </View>
                      <Ionicons name="chevron-down" size={18} color={slateTokens.textMuted} />
                    </TouchableOpacity>
                  ...
                  <View style={styles.formGroup}>
                    ...
                    <TouchableOpacity style={styles.selectBox} onPress={() => setIsGidaSelectOpen(true)}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <Ionicons name="restaurant-outline" size={18} color={slateTokens.textMuted} />
                        <Text style={styles.selectBoxText}>
                  ...
                  <View style={styles.formGroup}>
                    ...
                    <TouchableOpacity style={styles.selectBox} onPress={() => setIsIsgSelectOpen(true)}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <Ionicons name="shield-outline" size={18} color={slateTokens.textMuted} />
                        <Text style={styles.selectBoxText}>
                          {formIsg === 'D' ? 'DÜŞÜK' : (formIsg === 'O' ? 'ORTA' : 'YÜKSEK')}
                        </Text>
                      </View>
                      <Ionicons name="chevron-down" size={18} color={slateTokens.textMuted} />
                    </TouchableOpacity>
            ...
            >
              ...
              >
                ...
                <View style={styles.newDetailHeaderTopRow}>
                  ...
                  {(() => {
                    const statusStyle = getStatusStyle(selectedRequest.durum, !!selectedRequest.sorumluAd);
                    let badgeBg = '#e2e8f0';
              ...
              <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.detailScroll} showsVerticalScrollIndicator={false}>
                ...
                {/* TALEP DETAYLARI CARD (BAKIM Ozel Tasarim) */}
                {(() => {
                  ...
                  const getBoxColor = (val: any) => {
                    const v = (val || '').toUpperCase();
                    if (v.includes('YÜKSEK') || v.includes('DURDU') || v.includes('ACİL')) return '#ef4444'; // Red
                    if (v.includes('ORTA') || v.includes('BEKLEYEBİLİR')) return '#eab308'; // Yellow
                    return '#22c55e'; // Green for Düşük or others
                  };
                  ...
                  return (
                    <View style={styles.detailCard}>
                      ...
                      {/* Şirket / Bölüm */}
                      <View style={styles.infoRow}>
                        <Text style={[styles.infoRowLabel, { color: slateTokens.textDark, fontWeight: '700', marginLeft: 0 }]}>Şirket/Bölüm</Text>
                        <Text style={[styles.infoRowValue, { color: slateTokens.textSecondary, fontWeight: '500' }]}>
                          {selectedRequest.sirketAdi || '-'} / {selectedRequest.bolumAdi || '-'}
                        </Text>
                      </View>
                      <View style={[styles.detailDivider, { borderStyle: 'dashed' }]} />
                      ...
                      {/* Makine */}
                      <View style={styles.infoRow}>
                        <Text style={[styles.infoRowLabel, { color: slateTokens.textDark, fontWeight: '700', marginLeft: 0 }]}>Makine</Text>
                        <Text style={[styles.infoRowValue, { color: slateTokens.textSecondary, fontWeight: '500' }]}>
                          {selectedRequest.makineAdi || '-'}
                        </Text>
                      </View>
                      ...
                      {/* 3 Grid Boxes */}
                      {(() => {
                        const getUretimDurusLabel = (val: any) => val === 'EHAT' ? 'HAT DURDU' : val === 'EMAK' ? 'MAKİNE DURDU' : 'DURUŞ YOK';
                        const getGuvLabel = (val: any) => val === 'Y' ? 'YÜKSEK' : val === 'O' ? 'ORTA' : 'DÜŞÜK';
                        
                        const uretimVal = getUretimDurusLabel(selectedRequest.uretimDurusu);
                        const gidaVal = getGuvLabel(selectedRequest.gidaGuvOncelik);
                        const isGuvVal = getGuvLabel(selectedRequest.isGuvOncelik);
                        
                        return (
                          <>
                            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
                              <View style={{ flex: 1, borderWidth: 1, borderColor: getBoxColor(uretimVal) + '40', backgroundColor: getBoxColor(uretimVal) + '0A', borderRadius: 6, paddingVertical: 10, alignItems: 'center' }}>
                                <Text style={{ fontSize: 10, color: slateTokens.textMuted, fontWeight: '600', marginBottom: 4 }}>ÜRETİM DURUŞ</Text>
                                <Text style={{ fontSize: 11, color: getBoxColor(uretimVal), fontWeight: '700', textAlign: 'center' }}>{uretimVal}</Text>
                              </View>
                              <View style={{ flex: 1, borderWidth: 1, borderColor: getBoxColor(gidaVal) + '40', backgroundColor: getBoxColor(gidaVal) + '0A', borderRadius: 6, paddingVertical: 10, alignItems: 'center' }}>
                                <Text style={{ fontSize: 10, color: slateTokens.textMuted, fontWeight: '600', marginBottom: 4 }}>GIDA GÜV.</Text>
                                <Text style={{ fontSize: 11, color: getBoxColor(gidaVal), fontWeight: '700', textAlign: 'center' }}>{gidaVal}</Text>
                              </View>
                              <View style={{ flex: 1, borderWidth: 1, borderColor: getBoxColor(isGuvVal) + '40', backgroundColor: getBoxColor(isGuvVal) + '0A', borderRadius: 6, paddingVertical: 10, alignItems: 'center' }}>
                                <Text style={{ fontSize: 10, color: slateTokens.textMuted, fontWeight: '600', marginBottom: 4 }}>İŞ GÜV.</Text>
                                <Text style={{ fontSize: 11, color: getBoxColor(isGuvVal), fontWeight: '700', textAlign: 'center' }}>{isGuvVal}</Text>
                              </View>
                            </View>
                        ...
                        {isIsEmriExpanded && (detailData?.isEmriList || []).map((wo, i) => (
                            <View key={i} style={[styles.historyCard, { marginBottom: 12 }]}>
                              <View style={styles.historyCardHeader}>
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                  <View style={[styles.historyIconWrapper, { backgroundColor: slateTokens.primary + '1A' }]}>
                                    <Ionicons name="construct" size={16} color={slateTokens.primary} />
                                  </View>
                                  <Text style={styles.historyTitle}>{wo.isEmriTuru}</Text>
                                </View>
                                <Text style={styles.historyDate}>{wo.kayitTarStr}</Text>
                              </View>
                              <View style={styles.historyContent}>
                                <Text style={styles.historyDesc}>{wo.aciklama}</Text>
                                
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 12 }}>
                                  <View>
                                    <Text style={{ fontSize: 11, color: slateTokens.textMuted }}>Sorumlu:</Text>
                                    <Text style={{ fontSize: 13, color: slateTokens.text, fontWeight: '500' }}>{wo.adSoyad || 'Atanmadı'}</Text>
                                  </View>
                                  <View>
                                    <Text style={{ fontSize: 11, color: slateTokens.textMuted }}>Termin:</Text>
                                    <Text style={{ fontSize: 13, color: slateTokens.text, fontWeight: '500' }}>{wo.terminTarStr}</Text>
                                  </View>
                                  <View>
                                    <Text style={{ fontSize: 11, color: slateTokens.textMuted }}>Durum:</Text>
                                    <Text style={{ fontSize: 13, color: wo.durum ? slateTokens.success : slateTokens.warning, fontWeight: 'bold' }}>
                                      {wo.durum ? 'Kapalı' : 'Açık'}
                                    </Text>
                                  </View>
                                </View>
              ...
              <ScrollView keyboardShouldPersistTaps="handled" style={styles.modalBody}>
                ...
                <TouchableOpacity style={styles.formInput} onPress={() => setIsWoTurSelectOpen(true)}>
                  <Text style={{ color: woFormTur ? slateTokens.text : slateTokens.textMuted }}>
                    {woFormTur ? isEmriTurleri.find(t => t.isEmriTurID === woFormTur)?.tanim : 'Tür Seçin'}
                  </Text>
                </TouchableOpacity>
      ...
      <Modal visible={isWoTurSelectOpen} transparent animationType="fade" onRequestClose={() => setIsWoTurSelectOpen(false)}>
        <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={() => setIsWoTurSelectOpen(false)}>
          <View style={styles.dropdownContainer}>
            <View style={styles.dropdownHeader}>
              <Text style={styles.dropdownTitle}>Tür Seçin</Text>
            </View>
            <FlatList
              data={isEmriTurleri}
              keyExtractor={(item) => item.isEmriTurID.toString()}
              style={{ maxHeight: 300 }}
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.dropdownItem} onPress={() => { setWoFormTur(item.isEmriTurID); setIsWoTurSelectOpen(false); }}>
                  <Text style={[styles.dropdownItemText, woFormTur === item.isEmriTurID && { color: slateTokens.primary, fontWeight: '700' }]}>{item.tanim}</Text>
                  {woFormTur === item.isEmriTurID && <Ionicons name="checkmark-circle" size={20} color={slateTokens.primary} />}
                </TouchableOpacity>
              )}
            />
          </View>
      ...
      <Modal visible={isWorkOrderAssignModalOpen} transparent animationType="fade" onRequestClose={() => setIsWorkOrderAssignModalOpen(false)}>
        <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={() => setIsWorkOrderAssignModalOpen(false)}>
          <View style={styles.dropdownContainer}>
            <View style={styles.dropdownHeader}>
              <Text style={styles.dropdownTitle}>İş Emri Sorumlusu Seçin</Text>
            </View>
            <FlatList
              data={allActivePersonnelList}
              keyExtractor={(item) => item.sicilNo}
              style={{ maxHeight: 300 }}
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.dropdownItem} onPress={() => handleAssignWorkOrder(item.sicilNo)}>
                  <Text style={styles.dropdownItemText}>{item.adSoyad}</Text>
                </TouchableOpacity>
              )}
            />
          </View>
...
const createStyles = (colors: any, type: string, theme: string) => StyleSheet.create({
  ...
  bottomActionBtnOutline: {
    borderColor: colors.accent,
    backgroundColor: 'transparent',
  },
  bottomActionBtnOutlineText: {
    color: colors.accent,
  },
